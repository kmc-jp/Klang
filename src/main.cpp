...
int main(int argc, char** argv) {
  ...
  if(!success) {
    std::cerr << "someting wrong" << std::endl;
    if(tokens.size() != 0){
      std::cerr << "near " << tokens[tokens.size()-1].str() << std::endl; // helper_lexer のshow 系をもってきても良いかもしれない
    } else {
      std::cerr << "near first token" << std::endl;
    }
    exit(1);
  }
  ...
  std::unique_ptr<llvm::Module> module = generator.gen();
